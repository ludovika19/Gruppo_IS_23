...
public class CommentoDAO {
    ...
    public int getNumCommentiPerRicetta(){
        ...
        String query = "SELECT COUNT(c.idCommento) AS 'TotCommentiRicevuti'\n" +
                        "FROM instafood.Ricetta r\n" +
                        "JOIN instafood.Commento c ON r.idRicetta = c.Ricetta_idRicetta\n" +
                        "WHERE r.Utente_idUtente ='"+ this.utente_idUtente +"'';";
    ...
    public ArrayList<RicettaDTO> getRicettaWithMostComment(){
        ArrayList<RicettaDTO> listaRicetta = new ArrayList<>();
        ...
        try{
            ...
            while ((rs.next())){
                ...
                listaRicetta.add(DTOConverter.convertToDTO(ricetta_temp));
            }
        }catch ( ClassNotFoundException | SQLException e){
            e.printStackTrace();
        }
    ...
    public boolean createCommento(int idUtente, int idRicetta, String testo) {
        // Genero un ID univoco
        int nuovoId = (int) (System.currentTimeMillis() % 1000000);
        
        String query = "INSERT INTO instafood.Commento (idCommento, Testo, Data, Utente_idUtente, Ricetta_idRicetta) VALUES " +
                      "('" + nuovoId + "', '" + testo + "', CURDATE(), '" + idUtente + "', '" + idRicetta + "')";
    ...
    public List<entity.Commento> getCommentiByUtente(int idUtente) {
        List<entity.Commento> commenti = new ArrayList<>();
        String query = "SELECT * FROM instafood.Commento WHERE Utente_idUtente='" + idUtente + "';";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                commentoDAO.setIdCommento(rs.getInt("idCommento"));
                commentoDAO.setTesto(rs.getString("Testo"));
                commentoDAO.setData(rs.getDate("Data"));
                commentoDAO.setUtente_idUtente(rs.getInt("Utente_idUtente"));
                commentoDAO.setRicetta_idRicetta(rs.getInt("Ricetta_idRicetta"));
                
                entity.Commento commento = new entity.Commento(commentoDAO);
    ...
    public List<entity.Commento> getCommentiByRicetta(int idRicetta) {
        List<entity.Commento> commenti = new ArrayList<>();
        String query = "SELECT * FROM instafood.Commento WHERE Ricetta_idRicetta='" + idRicetta + "';";
